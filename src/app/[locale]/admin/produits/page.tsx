...
export default function ProduitsPage() {
  ...
  const handleSaveProduct = async () => {
    if (!productFormName.trim()) { setProductError(t("nameRequired")); return; }
    const priceCents = Math.round(parseFloat(productFormPrice) * 100);
    if (!priceCents || priceCents <= 0) { setProductError(t("pricePositive")); return; }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setProductError(data.error || tCommon("error"));
      }
    } catch { setProductError(tCommon("connectionError")); }
  ...
  const handleDeleteProduct = async (id: string) => {
    ...
    try {
      ...
      } else alert((await res.json()).error || tCommon("error"));
    } catch { alert(tCommon("connectionError")); }
  };
      ...
      <div className="flex items-center justify-between mb-2.5 px-1">
        <div className="flex items-center gap-2">
          ...
          {productsLoaded && adminProducts.length > 0 && (
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">{activeProductCount} {t("active")}</span>
          )}
        </div>
        <button
      ...
      <div className="bg-white rounded-2xl border border-gray-200/60 p-4 sm:p-5">
        {!productsLoaded ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500 mb-1">{t("noProducts")}</p>
            <p className="text-[11px] text-gray-400 mb-4">{t("addFirst")}</p>
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!p.active && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-gray-200 text-gray-500 uppercase">{t("inactive")}</span>}
                      </div>
                      <div className="flex items-center gap-3 mt-0.5">
                        <span className="text-sm font-bold text-gray-900">{new Intl.NumberFormat(locale === "en" ? "en-US" : "fr-FR", { style: "currency", currency: "EUR" }).format(p.price / 100)}</span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowProductForm(false)}>
            ...
            >
              <div className="sticky top-0 bg-white/90 backdrop-blur-xl border-b border-gray-100 px-5 py-4 flex items-center justify-between z-10">
                <h3 className="text-sm font-bold text-gray-900">{editingProduct ? t("editProduct") : t("newProduct")}</h3>
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{t("name")}</label>
                  <input
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{t("price")}</label>
                  <input
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{t("description")}</label>
                  <textarea
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{t("mainImage")}</label>
                  <input
                  ...
                  {productFormMainImg && (
                    <div className="mt-2 w-20 h-20 rounded-xl bg-gray-50 border border-gray-100 overflow-hidden">
                      <img src={productFormMainImg} alt={t("preview")} className="w-full h-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} />
                    </div>
                  )}
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider">{t("additionalImages")}</label>
                    <button onClick={addImageField} className="text-[10px] font-semibold text-blue-600 hover:text-blue-700">{t("addImage")}</button>
                  </div>
              ...
              <div className="sticky bottom-0 bg-white/90 backdrop-blur-xl border-t border-gray-100 px-5 py-4 flex items-center justify-end gap-2">
                ...
                <button onClick={handleSaveProduct} disabled={productSaving}
                  ...
                  {productSaving ? (
                    <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> {tCommon("saving")}</>
                  ) : (
