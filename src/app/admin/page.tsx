...
function AdminDashboardContent() {
  ...
  const fetchAdminProducts = useCallback(() => {
    // Fetch all products (including inactive) via a special admin endpoint
    // For now, we use the same GET but will need admin access
    // Actually, GET /api/products only returns active ones.
    // We'll add an admin query param
    fetch("/api/products?admin=all").then(r => r.json()).then(d => {
      // If admin=all fails, fallback to regular
